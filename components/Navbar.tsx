...
const Navbar: React.FC = () => {
  ...
  return (
    <header className={`fixed top-0 left-0 w-full z-[100] transition-all duration-300 py-6 bg-background ${scrolled ? 'shadow-md py-4' : ''}`}>
      <div className="row flex justify-between items-center">
        <a href="/" className="flex items-center gap-3 group">
          ...
          />
          <span className="text-[1.8rem] md:text-[2.2rem] font-bold tracking-tighter uc text-primary group-hover:text-accent transition-colors">Catering</span>
        </a>

        <nav className="flex items-center space-x-4 md:space-x-8">
          <div className="flex items-center space-x-6">
            <a href="/#food" className="text-[14px] md:text-[16px] font-medium uppercase tracking-wider hover:text-accent transition-colors">Food</a>
            <a href="/#get" className="text-[14px] md:text-[16px] font-medium uppercase tracking-wider hover:text-accent transition-colors">Order</a>
            <a href="/#locations" className="text-[14px] md:text-[16px] font-medium uppercase tracking-wider hover:text-accent transition-colors">Locations</a>
            <a 
          ...
          <a href="/cart" className="relative p-2 text-primary hover:text-accent transition-colors">
            <ShoppingCart className="w-6 h-6 md:w-7 h-7" />
            <span className={`absolute top-0 right-0 bg-accent text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full min-w-[18px] text-center ${totalItems > 0 ? 'block' : 'hidden'}`}>
              {totalItems}
            </span>
          </a>
        </nav>
